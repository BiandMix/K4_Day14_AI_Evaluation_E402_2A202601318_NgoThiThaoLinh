...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Evaluate the answer using the rubric below. Return only a JSON object "
        ...
        try:
            ...
            scores = {
                ...
                if criterion in candidate else 0.5
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        first_scores = [
            result.get("position", result.get("response_position"))
            for result in scores_batch
        ]
        positional_bias = False
        if any(position is not None for position in first_scores):
            by_position: dict[Any, list[float]] = {}
            for result, position in zip(scores_batch, first_scores):
                result_values = list(result.get("scores", {}).values())
                if position is not None and result_values:
                    by_position.setdefault(position, []).append(
                        sum(result_values) / len(result_values)
                    )
            if 1 in by_position and 2 in by_position:
                avg_first = sum(by_position[1]) / len(by_position[1])
                avg_second = sum(by_position[2]) / len(by_position[2])
                positional_bias = abs(avg_first - avg_second) > 0.1
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_averages = {metric: avg(new_results, metric) for metric in metrics}
        baseline_averages = {metric: avg(baseline_results, metric) for metric in metrics}
        regressions = [
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        fixes = {
            "hallucination": "Add claim-level grounding checks and reject unsupported policy claims",
            "irrelevant": "Improve intent routing and add prompt examples that answer the user's exact question",
            "incomplete": "Improve retrieval coverage and add a checklist for required conditions and exceptions",
            "off_topic": "Add intent validation and a scoped fallback before answer generation",
            "refusal": "Calibrate safety guardrails with valid support-request examples",
            "unknown": "Review low-scoring traces and add representative regression cases",
        }
        ordered = sorted(categories, key=lambda key: (-categories[key], key))
        return [fixes.get(category, fixes["unknown"]) for category in ordered]
